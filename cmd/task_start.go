...
func newTaskStartCmd() *cobra.Command {
	...
	cmd.Flags().StringVarP(&o.TaskToken, "task-token", "t", o.TaskToken, "task token; defaults to the enrolled task metadata")
...
func (o *taskStartOptions) prepare() (*taskStartRuntime, error) {
	...
	if taskGUID == "" {
		return nil, fmt.Errorf("task metadata does not include task_guid")
	}
	if strings.TrimSpace(metadata.TaskGUID) != "" && metadata.TaskGUID != taskGUID {
		return nil, fmt.Errorf("task metadata task_guid %q does not match requested task GUID %q", metadata.TaskGUID, taskGUID)
	}

	taskToken := strings.TrimSpace(o.TaskToken)
	if taskToken == "" {
